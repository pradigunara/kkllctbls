...
/* harmony import */ var lodash_find__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(6918);
/* harmony import */ var lodash_find__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(lodash_find__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var lodash_kebabCase__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(1546);
...
const pt = (...p) => path__WEBPACK_IMPORTED_MODULE_5___default().join(process.cwd(), ...p);

const dbPath = pt('/data/db.json');

const load = () => JSON.parse(fs__WEBPACK_IMPORTED_MODULE_4___default().readFileSync(dbPath).toString());

const write = db => fs__WEBPACK_IMPORTED_MODULE_4___default().writeFileSync(dbPath, JSON.stringify(db, null, 2));

const writeImg = (base64image, imgPath) => {
  const buffer = new Buffer(base64image, 'base64');
  fs__WEBPACK_IMPORTED_MODULE_4___default().writeFileSync(pt('/public', imgPath), buffer);
};
...
function handler(req, res) {
  ...
  fs__WEBPACK_IMPORTED_MODULE_4___default().copyFileSync(dbPath, pt('/data/db.json.bak'));
  const db = load();
  const storagePath = ['cards', memberCode, eraCode, sectionCode];

  const cards = lodash_get__WEBPACK_IMPORTED_MODULE_3___default()(db, storagePath, []);

  const imgPath = `/card/${memberCode}-${eraCode}-${sectionCode}-${lodash_kebabCase__WEBPACK_IMPORTED_MODULE_2___default()(name)}.jpg`;
  console.log('cc', cards);

  const existing = lodash_find__WEBPACK_IMPORTED_MODULE_1___default()(cards, {
    name
  });
  ...
  cards.push({
    ...
    code: lodash_kebabCase__WEBPACK_IMPORTED_MODULE_2___default()(name),
    name,
    memberCode,
    eraCode,
    sectionCode,
